...
from yolo.yolo_net_verclass import YOLONet
...
class Detector(object):
    ...
    def image_detector(self, imname, wait=0):
        ...
        image_dir_file = os.path.join(cfg.IMAGE_output_dir,'{}.jpg'.format(datetime.datetime.now().strftime('%H_%M:%S')))
        cv2.imwrite(image_dir_file, image)
        cv2.waitKey(wait)


def main():
    if not os.path.exists(cfg.IMAGE_output_dir):
        os.makedirs(cfg.IMAGE_output_dir)
