...
class Solver(object):
    ...
    def __init__(self, net, data):
        ...
        self.train_process_save_txt_file = os.path.join(cfg.TRAIN_process_save_txt_dir,
                                                        '{}_car_project.txt'.format(datetime.datetime.now().strftime('%d_%H:%M')))
        ...
        if 0:
            ...
            for i in range(len(self.variable_to_restore)):
                print('{}   {}\n{}'.format(self.restore_name[i].op.name,i,self.variable_to_restore[self.restore_name[i].op.name]))
